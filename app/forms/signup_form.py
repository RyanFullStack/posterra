...
class SignUpForm(FlaskForm):
    username = StringField(
        'username', validators=[DataRequired(), Length(min=2, max=40), username_exists])
    ...
    email = EmailField('email', validators=[DataRequired(), Email(), Length(min=2, max=255), user_exists])
